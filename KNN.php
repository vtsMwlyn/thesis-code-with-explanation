...
class KNN {
    ...
    public static function predict($currTemperature, $currPH, $currTurbidity, $currDissolvedSolids){
        ...
        // Buat nyimpen jarak data dari data baru dengan semua dataset + kualitas yang diklasifikasikan KNN
        $distances = [];

        // Ketika ada data baru yang datang, maka untuk setiap dataset (setiap kolom) kita lakukan...
        foreach($temperatures as $i => $temp){

            // Buat masing-masing suhu, pH, turbidity, TDS... cari selisih data yang baru dengan dataset saat ini, terus dikuadratin
            ...
            // Jumlahin semuanya, terus diakarin
            $distance = sqrt($temp2 + $pH2 + $tds2 + $turbidity2);

            // Simpen datanya ke list jarak + kualitas yang muncul
            array_push($distances, ['quality' => $labels[$i], 'distance' => $distance]);
        }

        // Diurutin semuanya dari kecil ke besar karena kita butuh tau top 15 data yang terdekat (nilai jaraknya kecil)
        usort($distances, fn($a, $b) => $a['distance'] <=> $b['distance']);

        // Ambil top 15 data
        $nearestNeighbors = array_slice($distances, 0, $k);

        // Cari label (very bad, bad, moderate, good, excellent) yang terbanyak dari ke-15 data yang kita dapat, urutin yang paling banyak taro depan
        $qualities = array_count_values(array_column($nearestNeighbors, 'quality'));
        arsort($qualities);

        // Label terbanyak adalah label untuk data yang baru dan karena sebelumnya udah diurutin sedemikian rupa, tinggal ambil yang depan aja
        return array_key_first($qualities);
    }

    // ===== BUAT GENERATE CONFUSION MATRIX UNTUK AKURASI SI KNN ===== //
    public static function confusion_matrix(){
        // Dataset test (30 data)
        $temperatures = [
        ...
        // Buat nyimpen "Jawaban"
        $predicted_labels = [];

        // Dengan dataset 170 biji, KNN-nya sendiri ngeklasifikasiin 30 data itu kayak gimana?
        foreach($temperatures as $i => $temp){
            $predicted_labels[] = KNN::predict($temp, $phs[$i], $tdss[$i], $turbidities[$i]);
        }

        // Kirim datanya ke depan biar bisa di-map jadi tabel
        return [
            'true' => $true_labels,
